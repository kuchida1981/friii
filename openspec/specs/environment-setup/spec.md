## Purpose
この仕様は、プロジェクト「friii」の開発を円滑に進めるための Docker Compose による統合開発環境、バックエンドのクリーンアーキテクチャ構成、およびフロントエンドの初期プロジェクト構成を詳細に定義する。

## Requirements

### Requirement: Docker Compose による統合開発環境の提供
システムは Docker Compose を使用して、バックエンド、フロントエンド、およびデータベースが相互に通信可能な統合開発環境を提供しなければならない (SHALL)。
...
バックエンドは Clean Architecture の原則に従い、`internal/` 配下に `domain`, `usecase`, `infrastructure`, `interface` の各レイヤーを分離して保持しなければならない (SHALL)。
...
フロントエンドは Vite と TypeScript を使用して構築され、バックエンドの GraphQL エンドポイントと通信するための基本設定を保持しなければならない (SHALL)。
...
システムは、開発環境セットアップの一部として `pre-commit` のインストールおよびフックの登録手順を `DEVELOPMENT.md` 等に明記しなければならない (SHALL)。
